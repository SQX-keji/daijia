<template>
	...
	<view class="register">
		...
		<view class="footer" style=" margin-right: 72px;">
			...
			<navigator style="font-size: 28upx;" url="/my/setting/xieyi" open-type="navigate">《用户协议》</navigator>
		</view>
	...
	<view v-if="isCanUse">
		<view>
			...
			<button v-show="weixinPhone" style="background: #346EF6;color: #FFFFFF;" class="bottom"
			...
			<view class="footer" style="font-size: 32upx;">
				...
				<navigator url="/my/setting/xieyi" open-type="navigate" style="font-size: 32upx;">《用户协议》
				</navigator>
			</view>
		</view>
	</view>
		...
		methods: {
			...
			getWeixinInfo(nickName, avatarUrl) {
				...
				if (!token) {
					...
					that.$Request.postJson("/app/Login/insertWxUser", {
						...
						phone: that.phone,
						inviterCode: that.$queue.getData("userByinvitationId") ? that.$queue.getData(
							"userByinvitationId") : ''
					}).then(res => {
						if (res.code === 0) {
							uni.hideLoading();
			...
			startReg() {
				...
				that.$Request.postJson("/app/Login/registerCode?password=" + that.passData + '&phone=' + that.phoneData +
					...
					'&inviterCode=' + inviterCode).then(res => {
					console.log(res)
					if (res.code == 0) {
